% Example file for embedding 11 manifolds from COIL-20 dataset into 2D
...
Y = []; t = [];
for i=1:length(M)
  Y = [Y; Y0(t0==M(i),:)]; t = [t; t0(t0==M(i),:)];
end
N = size(Y,1);

% compute the positive Gaussian affinities with adaptive bandwidth using
% perplexity k = 20.
Wp = ea(Y,20);
%Wp = x2p(Y',20);	slower
